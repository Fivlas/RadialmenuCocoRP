...
RegisterKeyMapping('_rad', 'Open Radial Menu', 'keyboard', 'F1')
...
AddEventHandler('qb-radialmenu:client:noPlayers', function(data)
    ...
    exports['mythic_notify']:SendAlert('normal', 'There arrent any people close')
end)
    ...
    -- if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                exports['mythic_notify']:SendAlert('normal', 'Your now on the  '..data.title..'!')
            else
                --QBCore.Functions.Notify('The vehicle goes to fast..')
                exports['mythic_notify']:SendAlert('normal', 'The vehicle goes too fast..')
            end
        else
            --QBCore.Functions.Notify('This seat is occupied..')
            exports['mythic_notify']:SendAlert('normal', 'This seat is occupied..')
        end
    -- else
